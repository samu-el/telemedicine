...
#### Module 1.2.1 — Authentication & Identity Management

**Patient Registration Flow:**
1. Patient enters their email address and chooses a password meeting strict complexity rules (minimum 12 characters, mix of case, numerals, and symbols).
2. A Double Opt-In (DOI) email is dispatched containing a time-limited (24-hour) verification link.
3. Upon email verification, the patient is prompted to complete their basic profile (legal name, date of birth, address — all fields encrypted at rest).
4. Age verification gate: patients must confirm they are 18 years or older. Future expansion may support minors with guardian consent flows.
5. Account is created in `is_active: false` state until DOI is confirmed.
...
- Each questionnaire belongs to a **Treatment Type** (e.g., "Hair Loss Treatment", "Contraception Consultation", "Allergy Assessment"). A patient selects their treatment type at the start of the intake flow.
...
- All in-progress answers are auto-saved to the backend every 30 seconds.
...
| :--- | :--- | :--- |
| `Draft` | Patient has begun the intake but not submitted. | System (auto-created on first questionnaire save) |
| `Submitted` | Patient has completed the questionnaire and uploaded docs. | Patient |
...
**Payment Flow:**
1. After questionnaire + document submission, the patient lands on a payment summary screen showing the service fee.
...
- Cohort analysis: patient acquisition by treatment type, conversion funnel (Registration → Questionnaire → Payment → Approved).
...
The following describes the complete patient-to-fulfillment lifecycle in sequential operating steps:

1. **Registration & DOI Verification**: Patient creates account → verifies email → completes encrypted profile.
2. **Mandatory Consent Collection**: Patient accepts all required consent documents (current versions, individually timestamped and IP-logged).
3. **Treatment Type Selection**: Patient selects the treatment category they are seeking (e.g., "Hair Loss").
4. **Dynamic Questionnaire Completion**: Patient progresses through branching questionnaire. Any hard-stop triggers a clinical halt message; no case or charge is created.
5. **Document Upload**: Patient uploads any required supporting documents (ID, previous test results, photos).
6. **Payment**: Patient reviews fee and completes payment via Stripe. Invoice is auto-generated.
7. **Case Creation & Queueing**: System creates a `Case` in `Paid` status, assigned to the relevant specialty queue.
8. **Doctor Claim**: A licensed, available Doctor claims the case from the queue, triggering `In Review` status.
9. **Doctor Review**: Doctor evaluates the complete case file, messaging the patient if clarification is needed.
10. **Medical Decision (Approve / Reject / More Info)**: Doctor records their decision and mandatory justification. All records immutably written.
11. **Patient Notification**: Patient receives an email notification of outcome, directing them to the portal.
12. **Prescription Dispatch (if Approved)**: A structured, signed prescription payload is forwarded to the designated Partner.
13. **Pharmacy Fulfillment**: Partner acknowledges, prepares, and ships the order. Status updates are pushed back to the platform.
14. **Delivery & Case Closure**: Upon confirmed delivery, the case transitions to `Completed`.
15. **Audit Finalization**: The complete case lifecycle — every event, access, change, and decision — is durably stored in the immutable Audit Log.
...
*Expected deliverables during Discovery mapping out the Patient Onboarding, Dynamic Questionnaire Flow, Doctor Dashboard with the Decision Workflow panel, Pharmacy Order Tracker, and the Admin CMS/Reporting Panel.*
...
| **3** | UX & questionnaire | Design system foundations (Tailwind tokens); wireframes and flows: patient registration → double opt-in → consent → treatment selection → questionnaire → documents → payment; doctor queue → claim → review → decision; admin consent and questionnaire CMS; accessibility target (WCAG 2.1 Level AA aspirational). |
| **4** | Technical foundations | Finalise ERD against §3; first OpenAPI stubs for `auth`, `consents`, `forms`, `cases`; Terraform modules for VNet, Container Apps, PostgreSQL Flexible Server, Redis, Blob Storage, Key Vault (§2.7); **Staging** environment with empty database; CI: lint (Flake8/Black), pytest, Docker image build, deploy to Staging. |

#### 5.3.2 Phase 1 Deliverables (Exit Criteria)
- Signed-off **MVP backlog** with stories traceable to business rules BR-01–BR-10 (§1.3).
...
- **Backend**: FastAPI project structure; SQLAlchemy 2.0 async models; `User`, `Role`; JWT access and refresh tokens; password hashing (bcrypt cost ≥ 12); email verification tokens for double opt-in; Alembic migrations; `/health` and `/ready` endpoints.
- **Frontend**: Next.js 15 App Router shell; patient registration and login; React Query for API calls; protected routes using session/JWT pattern aligned with §2.5.2.
- **Infra**: Deploy API and web to Staging; environment variables from Key Vault references.
- **Demo**: Patient completes registration and email verification; authenticated call to protected endpoint; doctor self-registration rejected at API level.
...
#### 5.4.5 Sprint 5 (Weeks 13–14) — Case State Machine & Payments
- **Backend**: `Case` and `CaseStatusHistory`; state transition validation (§1.2.5); Stripe PaymentIntent creation server-side; webhook endpoint with signature verification and idempotency; transition to `Paid`; Celery jobs for PDF invoice (Rechnung) and transactional emails without PHI in body (§1.2.8); email provider integration.
- **Frontend**: Payment summary; Stripe Checkout or Payment Element; patient case timeline.
- **Demo**: Happy path from `Submitted` through payment to `Paid` with invoice artifact; failed payment leaves case in `Submitted` (§1.2.5).
...
- [ ] Business rules BR-01–BR-10 enforced or explicitly waived with written risk acceptance.
